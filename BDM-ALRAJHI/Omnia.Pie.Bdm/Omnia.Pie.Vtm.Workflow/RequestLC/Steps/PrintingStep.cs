namespace Omnia.Pie.Vtm.Workflow.RequestLC.Steps
{
	...
	public class PrintingStep : WorkflowStep
	{
		...
		{
            _logger?.Info($"Execute Step: Printing LC");
			...
			var receipt = new RequestLCReceipt
			{
				...
				TransactionNumber = Context.Get<IRequestLCContext>()?.TSNno,
				TransactionStatus = TransactionStatus.Succeeded,
				TransactionCurrency = TerminalConfiguration.Section?.Currency,
				ReferenceNo = Context.Get<IRequestLCContext>()?.TSNno,
				VTMID = TerminalConfiguration.Section?.Id
			};

			_journal.TransactionSucceeded(Context.Get<IRequestLCContext>()?.TSNno);
