...
{
    public class CashDepositToAccountWorkflow : Workflow
    {
        ...
        public CashDepositToAccountWorkflow(IResolver container, IReceiptFormatter receiptFormatter) : base(container)
        {
            _journal.TransactionStarted(EJTransactionType.Financial, "Cash Deposit to Account - SS");
            ...
            _depositConfirmationStep.ExpiredAction = () =>
            {
                IgnoreExceptions(async () =>
                {
            ...
            _depositConfirmationStep._depositConfirmedAction = async () =>
            {
                ...
                try
                {
                    ...
                    var depositedDenominations = _context.DepositedCash.Select(x => new DepositedDenominations()
                    {
                        ...
                        .ToList();
                    var cashDeposited = new CashDeposited                                                   // hardcoded
                    {
                    ...
                    Context.Get<ICashDepositContext>().AuthCode = _cashDepositToAccount.ResponseCode;
                    ...
                    var transaction = (new DeviceTransaction
                    {
                        ...
                        isOffline = 0,
                        isUploaded = 0,
                        OnlineAuthCode = Context.Get<ICashDepositContext>().AuthCode,
                    });
                    bool isSaved = await _transactionStore.SaveTranasactionLocally(transaction, cashDeposited, depositedDenominations);

                    Context.Get<ICashDepositContext>().AuthCode = _cashDepositToAccount.TransactionId;
                ...
                catch (HttpRequestException ex)
                {
                    _logger.Info($"Error [Exception] Result: {ex.Message}");
                    _journal.TransactionFailed(ex.GetBaseException().Message);
                    //_logger.Exception(ex.GetBaseException());
        ...
        public async Task<bool> ExecuteAsync()
        {
            ...
            _journal.TransactionStarted(EJTransactionType.Financial, "Cash Deposit To Account - Online - ");
            ...
            try
            {
                    ...
                    if (await _checkReceiptPrinterStep.CheckPrinterAsync())
                    {
                        ...
                        //var temp = Context.Get<IDataContext>();
        ...
        private async Task<List<Services.Interface.Entities.Account>> LoadAccounts()
        {
            ...
            List<Services.Interface.Entities.Account> accounts;
            string transactionNumber = ConfigurationManager.AppSettings["transactionNumber"].ToString(); //Hardcoded 

            var _authenticationService = _container.Resolve<IAuthenticationService>();
            accounts = await _authenticationService.GetAccounts(customerIdentifier: _container.Resolve<ISessionContext>().CustomerIdentifier, Username: userInfo.Username, TransactionNumber: transactionNumber, conditionId: AccountCriterion.Casa);
